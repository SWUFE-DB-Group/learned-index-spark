...
public class APIndexBuild {
    public static void APIndexBuild(String[] args) throws Exception {
        ...
        JavaRDD<Point> partitionRDD = SpatialPartition.AdaptiveGridPartitioner(pointRDD);
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter("results/" + args[0] + "_build_results.txt", true))) {
            writer.write("--------------------------------------");
            writer.write(args[0]);
            writer.write("build APIndexBuild:"+"\n");
